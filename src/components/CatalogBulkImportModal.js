...
import { commitCatalogImport, downloadCatalogTemplate, previewCatalogImport } from '../services/adminApi';
...
export default function CatalogBulkImportModal({ token, industries, onClose, onImportSuccess }) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await downloadCatalogTemplate(token);
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 20 }}>Catalog Bulk Import</h3>
            <p style={{ margin: '4px 0 0', color: 'var(--text-muted, #6b7280)', fontSize: 14 }}>
              Import Main Category → Category → Sub-Category in one shot via CSV.
            </p>
          </div>
                  ...
                  {(previewData.rows || []).length === 0 ? (
                    ...
                    (previewData.rows || []).map((row) => (
                      <tr key={row.rowNumber} style={{ background: row.hasError ? '#fff1f2' : undefined }}>
                        ...
                        <td>
                          {(row.errors || []).map((e, i) => (
                            <span key={i} style={{ color: '#dc2626', fontSize: 12, display: 'block' }}>{e}</span>
                          ))}
                          {(row.warnings || []).map((w, i) => (
                            <span key={i} style={{ color: '#d97706', fontSize: 12, display: 'block' }}>{w}</span>
                          ))}
                        </td>
                      </tr>
                    ))
